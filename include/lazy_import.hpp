...
inline INLINE uint8_t *FindExportFromModule(std::uint8_t *address, const std::uint32_t hash) {
    if (!address) {
...
inline INLINE HMODULE FindModule(std::uint32_t hash) {
    auto peb_ptr = peb();
...
template <std::uint32_t Hash> struct ImportModule {
    using Type = HMODULE;
    INLINE auto resolve() -> Type { return reinterpret_cast<Type>(FindModule(Hash)); }
    INLINE auto cached() -> Type {
...
#else
        Type expected{};
        if (__atomic_compare_exchange_n(&cached_mod, &expected, result, false, __ATOMIC_RELEASE, __ATOMIC_RELAXED)) {
            return result;
        } else {
            return expected;
        }
...
template <typename Fn, std::uint32_t ModHash, std::uint32_t Hash> struct ImportFn {
    ...
    INLINE auto resolve() -> Type {
        auto result = FindExportFromModule(reinterpret_cast<std::uint8_t *>(ImportModule<ModHash>{}.resolve()), Hash);
        return reinterpret_cast<Type>(result);
    }
    INLINE auto cached() -> Type {
#ifdef _MSC_VER
...
#else
        Type expected{};
        if (__atomic_compare_exchange_n(&cached_value, &expected, result, false, __ATOMIC_RELEASE, __ATOMIC_RELAXED)) {
            return result;
        } else {
            return expected;
        }
    ...
    template <typename... Args> INLINE auto operator()(Args... args) -> std::invoke_result_t<Fn, Args...> {
...
#define ModHash(x) Fnv1aHashByLower(L## #x, sizeof(L## #x) / sizeof(wchar_t) - 1)
#define FnHash(x) Fnv1aHash(#x, sizeof(#x) - 1)
#define LazyImportMod(x) ImportModule<ModHash(x)>()
#define LazyFn(mod, fn) ImportFn<decltype(fn), ModHash(mod), FnHash(fn)>()
